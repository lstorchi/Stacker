...
def move_mindist_vers (xlist, ylist, zlist, atoms, s):
   ...
   print "Initial dist : ", d, " == ", dists[i, j]
   ...
   for i in range(len(xlist1)):
       xlist1[i] = xlist1[i] + u[0] * md
       ylist1[i] = ylist1[i] + u[1] * md
       zlist1[i] = zlist1[i] + u[2] * md
...
if (len(sys.argv)) == 3:
  ...
  print "usage :", sys.argv[0] , " xyzfile distance"
  exit(1)

xlist, ylist, zlist, atoms = xyznanop.read_ncxyz (filename, False)

xlist, ylist, zlist, atoms = \
        move_mindist_vers (xlist, ylist, zlist, atoms, dist)
