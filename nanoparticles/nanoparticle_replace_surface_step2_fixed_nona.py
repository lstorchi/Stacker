...
def set_fixed_for_touch_nanops (nanop, nanoparticles):
  ...
  for i in range(len(indexs)):
    if (nanop.nanoparticle_touch_me (nanoparticles[indexs[i]])):
      nanoparticles[indexs[i]].set_fixed()
...
for selectedid in range(len(nanoparticles)):
  ...
  if (not nanop_selected.get_fixed()):
    ...
    # set fixed le nanoparticelle che toccano questa puo' succede poi che cambi 
    # quando muovo le altre ma provo 
    set_fixed_for_touch_nanops (min_nanop, nanoparticles)
  else:
    new_nanoparticles_list.append(nanop_selected)

for nanop in new_nanoparticles_list:
  pcx, pcy, pcz = nanop.get_center()
  A, B, H = nanop.get_dimensions()
